...
This fork supports any provider implementing the OpenAI-compatible
`POST /chat/completions` API. Open **Settings → Neural Networks → AI provider**
and configure:

- Base URL, including the API version prefix (for example `https://api.openai.com/v1`)
- model name as exposed by the provider
- API key (optional for providers that do not require authentication)
- system prompt

Enable the provider and restart `jarvis-app`. Built-in commands keep priority;
when no command matches, Jarvis sends the recognized text to the configured AI
provider and displays the answer in the main window. API keys are saved only in
the user's local settings file and are never required by the GitHub build.

### Build with GitHub Actions

Fork the repository, push it to GitHub, then open **Actions → Build Windows →
Run workflow**. Download `jarvis-windows-x64` from the completed workflow's
Artifacts section. Do not add provider keys to the repository or workflow.
