<script lang="ts">
    ...
    let llmModel = ""
    let llmSystemPrompt = "You are Jarvis, a concise and helpful voice assistant. Reply in the user's language."
    ...
    async function saveSettings() {
        ...
        try {
            await Promise.all([
                ...
                invoke("db_write", { key: "llm_enabled", val: llmEnabled.toString() }),
                invoke("db_write", { key: "llm_base_url", val: llmBaseUrl }),
                invoke("db_write", { key: "llm_model", val: llmModel }),
                invoke("db_write", { key: "llm_system_prompt", val: llmSystemPrompt })
            ])
    ...
    onMount(async () => {
        ...
        try {
            ...
            const [mic, wakeWord, intentReco, slotEngine, glinerModel, voskModel,
                   ...
                   pico, openai, aiEnabled, aiBaseUrl, aiModel, aiSystemPrompt] = await Promise.all([
            ...
            apiKeyOpenai = openai
            llmEnabled = aiEnabled === "true"
            llmBaseUrl = aiBaseUrl || "https://api.openai.com/v1"
            llmModel = aiModel
            llmSystemPrompt = aiSystemPrompt
        } catch (err) {
    ...
    <Tabs.Tab label={t('settings-neural-networks')} icon={Cube}>
        ...
        <NativeSelect
            data={[
                ...
                { label: "Embedding Classifier", value: "embedding-classifier" }
            ]}
        ...
        <InputWrapper label={t('settings-ai-provider')}>
            <Text size="sm" color="gray">
                {t('settings-ai-provider-desc')}
            </Text>
            <Space h="sm" />
            <Switch label={llmEnabled ? t('settings-enabled') : t('settings-disabled')} bind:checked={llmEnabled} />
            <Space h="sm" />
            <Input icon={Code} placeholder="https://api.openai.com/v1" variant="filled" autocomplete="off" bind:value={llmBaseUrl} />
            <Space h="sm" />
            <Input icon={Code} placeholder={t('settings-ai-model')} variant="filled" autocomplete="off" bind:value={llmModel} />
            <Space h="sm" />
            <Input
                icon={Code}
                placeholder={t('settings-ai-key')}
                variant="filled"
                autocomplete="off"
                type="password"
            ...
            />
            <Space h="sm" />
            <Input icon={Code} placeholder={t('settings-ai-system-prompt')} variant="filled" bind:value={llmSystemPrompt} />
        </InputWrapper>
    </Tabs.Tab>
</Tabs>
