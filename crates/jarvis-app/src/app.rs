...
fn execute_command(text: &str, rt: &tokio::runtime::Runtime) -> bool {
    ...
    if let Some((cmd_path, cmd_config)) = cmd_result {
        ...
        info!("No command found for: {}, trying configured AI provider", text);
        match rt.block_on(jarvis_core::llm::chat(text)) {
            Ok(answer) => {
                ipc::send(IpcEvent::AssistantResponse { text: answer });
            }
            Err(message) => {
