...
fn default_llm_system_prompt() -> String {
    "You are Jarvis, a concise and helpful voice assistant. Reply in the user's language.".to_string()
}
...
impl Settings {
    ...
    pub fn set(&mut self, key: &str, val: &str) -> Result<(), String> {
        match key {
            ...
            "llm_enabled" => {
                self.llm_enabled = val.parse::<bool>()
                    .map_err(|_| format!("expected 'true' or 'false', got: '{}'", val))?;
            }
            "llm_base_url" => self.llm_base_url = val.trim().trim_end_matches('/').to_string(),
            "llm_model" => self.llm_model = val.trim().to_string(),
            "llm_system_prompt" => self.llm_system_prompt = val.to_string(),
            "api_key__picovoice" => {
