...
settings-ai-provider-desc = Any provider compatible with the OpenAI Chat Completions API. Local commands are tried first; unknown requests are sent to this provider.
settings-ai-key = API key (leave empty if the provider does not require one)
settings-ai-model = Model name
settings-ai-system-prompt = System prompt
