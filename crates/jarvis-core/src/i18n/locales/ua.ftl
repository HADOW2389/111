...
# settings - openai
settings-ai-provider = ШІ-провайдер
settings-ai-provider-desc = Будь-який провайдер, сумісний з OpenAI Chat Completions API. Спочатку виконуються локальні команди, інші запити надсилаються провайдеру.
settings-ai-key = API-ключ (залиште порожнім, якщо він не потрібен)
settings-ai-model = Назва моделі
settings-ai-system-prompt = Системний промпт
