...
# settings - openai
settings-ai-provider = ИИ-провайдер
settings-ai-provider-desc = Любой провайдер, совместимый с OpenAI Chat Completions API. Сначала выполняются локальные команды, остальные запросы отправляются провайдеру.
settings-ai-key = API-ключ (оставьте пустым, если он не требуется)
settings-ai-model = Название модели
settings-ai-system-prompt = Системный промпт
